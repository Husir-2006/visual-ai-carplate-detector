...
项目仓库：https://github.com/Husir-2006/visual-ai-carplate-detector
...
- 管理模块：车辆档案支持新增与编辑，黑白名单支持新增、编辑和状态切换；通行记录与系统设置均通过 JSON 接口驱动。
- 自动业务闭环：识别到新车牌后自动建立待完善车辆档案，并同步写入通行记录，后续可在管理界面人工核验和补充信息。
...
- `/api/fleet`：车辆档案查询、新增和编辑。
- `/api/pass-records`：通行记录数据。
- `/api/access-lists`：黑白名单查询、新增和编辑。
- `/api/settings`：系统设置数据。
...
visual-ai-campus-detector/
  ...
  ../Z24281098.zip               最终提交包（位于项目文件夹上一级）
```
...
压缩包包含源码、模型、系统数据、课程报告、PPT、功能演示视频、答辩讲解视频、讲稿、开发日志、沟通记录、个人实习实训日志和运行截图；不包含原始数据集、训练缓存、上传缓存和临时工作区。
